...
class TransformT(object):
    ...
    def pil_transformer(self, probability, level):
        def return_function(image=None, level=0):
            #print("*"*20)
            #print("TAlb")
            #print(self.name)
            if random.random() < probability:
                image = self.xform(image, level)
            #print("ALBed")
            return image
...
flip_lr = TransformT(
    'FlipLR',
    lambda pil_img, level: alb.transforms.HorizontalFlip(Image.FLIP_LEFT_RIGHT))
flip_ud = TransformT(
    'FlipUD',
    lambda pil_img, level: alb.transforms.VerticalFlip(Image.FLIP_TOP_BOTTOM))
# pylint:disable=g-long-lambda
auto_contrast = TransformTPIL(
    'AutoContrast',
    lambda pil_img, level: _autocontrast(pil_img, level))
equalize = TransformT(
    'Equalize',
    lambda pil_img, level: alb.functional.equalize(pil_img))
invert = TransformT(
    'Invert',
    lambda pil_img, level: alb.transforms.InvertImg(
        pil_img))
# pylint:enable=g-long-lambda
blur = TransformTPIL(
    'Blur', lambda pil_img, level: _blur_impl(pil_img, level))
smooth = TransformTPIL(
    'Smooth',
    lambda pil_img, level: _smooth_impl(pil_img, level))

def _autocontrast(pil_img, level):
    pil_img = ImageOps.autocontrast(pil_img)
    return pil_img

def _smooth_impl(pil_img, level):
...
def _rotate_impl(image, level):
    """Rotates `pil_img` from -30 to 30 degrees depending on `level`."""
    degrees = int_parameter(level, min_max_vals.rotate.max)
    if random.random() > 0.5:
        degrees = -degrees
    return cv2.warpAffine(image,rot_mat[degrees],(224,224))
...
def _posterize_impl(pil_img, level):
    ...
    posterize = alb.transforms.Posterize(num_bits=min_max_vals.posterize.max - level, always_apply=True)
    return posterize(image=pil_img)["image"]
...
shear_x = TransformTPIL('ShearX', _shear_x_impl)
...
shear_y = TransformTPIL('ShearY', _shear_y_impl)
...
translate_x = TransformTPIL('TranslateX', _translate_x_impl)
...
translate_y = TransformTPIL('TranslateY', _translate_y_impl)


def _crop_impl(img, level, interpolation=Image.BILINEAR):
...
def _solarize_impl(pil_img, level):
  ...
  """
    ...
    return  alb.functional.solarize(pil_img, threshold= 256 - level)
...
color = TransformTPIL('Color', _enhancer_impl(ImageEnhance.Color))
ohl_color = TransformTPIL('Color', _enhancer_impl(ImageEnhance.Color, .3, .9))
contrast = TransformTPIL('Contrast', _enhancer_impl(ImageEnhance.Contrast))
brightness = TransformTPIL('Brightness', _enhancer_impl(
    ImageEnhance.Brightness))
sharpness = TransformTPIL('Sharpness', _enhancer_impl(ImageEnhance.Sharpness))
contour = TransformTPIL(
    'Contour', lambda pil_img, level: pil_img.filter(ImageFilter.CONTOUR))
detail = TransformTPIL(
    'Detail', lambda pil_img, level: pil_img.filter(ImageFilter.DETAIL))
edge_enhance = TransformTPIL(
    'EdgeEnhance', lambda pil_img, level: pil_img.filter(ImageFilter.EDGE_ENHANCE))
sharpen = TransformTPIL(
...
mirrored_color = TransformTPIL('Color', _mirrored_enhancer_impl(ImageEnhance.Color))
mirrored_contrast = TransformTPIL('Contrast', _mirrored_enhancer_impl(ImageEnhance.Contrast))
mirrored_brightness = TransformTPIL('Brightness', _mirrored_enhancer_impl(
    ImageEnhance.Brightness))
mirrored_sharpness = TransformTPIL('Sharpness', _mirrored_enhancer_impl(ImageEnhance.Sharpness))
...
cutout = TransformTPIL('Cutout', lambda img, l: CutoutDefault(img, int_parameter(l, img.size[0] * min_max_vals.cutout.max)))
...
sample_pairing = TransformTPIL('SamplePairing', blend)


def set_augmentation_space(augmentation_space, num_strengths, custom_augmentation_space_augs=None):
    global ALL_TRANSFORMS, min_max_vals, PARAMETER_MAX, rot_mat
    ...
    cols=224
    rot_mat = {}
    for i in range(0, PARAMETER_MAX+1):
        degrees = int_parameter(i, min_max_vals.rotate.max)
        rot_mat[degrees] = cv2.getRotationMatrix2D((cols/2,rows/2),degrees,1)
        rot_mat[-degrees] = cv2.getRotationMatrix2D((cols/2,rows/2),-degrees,1)
...
class UniAugmentWeighted(ImageOnlyTransform):
    ...
    def apply(self, image=None, **kwargs):
        ...
        image = op.pil_transformer(1., level)(image)
        return image
